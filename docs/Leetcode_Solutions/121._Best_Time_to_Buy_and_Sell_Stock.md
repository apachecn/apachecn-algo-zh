...
对于从第一天后的每一天，我们都假设之前就已经卖掉了，那么现在我们可以看看**如果之前卖可以赚的钱加上今天与之前卖当天的价格差价**是否大于0，如果大于0就说明今天卖赚的更多，那么我们就还是先存下今天卖可以多赚的max_cur，继续往后看，如果小于0就还是坚持之前的价格（即之前的max_cur）
...
class Solution(object):
    def maxProfit(self, prices):
        ...
        for i in range(1, len(prices)):
            max_cur = max(0, max_cur+prices[i]-prices[i-1])  # 如果今天卖相比于以前卖能多赚，则今天卖，否则之前就卖了
            res = max(res, max_cur)
        return res
```
